...
void CArcCmdLineParser::Parse2(CArcCmdLineOptions &options)
{
  ...
  options.PasswordEnableForced = parser[NKey::kPasswordForced].ThereIs;
  if (options.PasswordEnableForced) {
	  CSwitchResult result = parser[NKey::kPasswordForced];
	  options.PasswordForcedDict = parser[NKey::kPasswordForced].PostStrings[0];
  }
  #endif
