...
HRESULT CExtractCallbackConsole::GetLastError() {
	if (GetPasswordForcedMax()) {
		return (long)-1;
	}
	return (long)this->_opRes;
}
...
HRESULT CExtractCallbackConsole::SetPasswordForced(const UString & isForced) {
	PasswordIsDefinedForced = true;
	PasswordDict = isForced;
	return S_OK;
}

HRESULT CExtractCallbackConsole::GetPasswordForced(){
	return PasswordIsDefinedForced;
}
